...
L'observation pratique reste utile : **le même pipeline de preuves SourceShelf a produit des réponses nettement meilleures sur macOS 27.** Vous pouvez voir le flux de travail de recherche sous-jacent dans notre [Exemple de planification d'un voyage au Japon](/examples/japan-trip-ai-planner/).
...
Notre [guide d’Open Knowledge Format](what-is-open-knowledge-format-okf.md) explique comment Markdown, les métadonnées, la provenance et un index rendent ces connaissances portables. La [spécification OKF](https://github.com/GoogleCloudPlatform/knowledge-catalog/blob/main/okf/SPEC.md) décrit le format lui-même.
