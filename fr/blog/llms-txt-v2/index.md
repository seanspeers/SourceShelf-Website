...
La proposition `llms.txt` a atteint la version 2. Elle met à jour cette convention émergente afin d’aider les assistants et agents IA à trouver du contenu Web utile et lisible par machine. Si vous découvrez ce format, commencez par notre [introduction à llms.txt](/fr/blog/what-is-llms-txt/).
...
La version 1.0.2 étend le flux de travail vers Safari, permettant à la collection `llms.txt` d'un site Web compatible de devenir le point de départ d'un Pack de recherche local. Le document [Guide SourceShelf llms.txt](/fr/docs/guides/llms-txt/) décrit l'ordre complet de découverte, le flux de travail de sélection, les mesures de sécurité et la provenance que l'application préserve.
...
Vous pouvez le conserver comme archive de recherche portable, exporter Markdown pour une autre application, créer un [Paquet de référence orienté IA](/fr/local-ai-reference-packs/) ou exposer un pack sélectionné à un client IA compatible via l'intégration locale et en lecture seule de MCP de SourceShelf.
