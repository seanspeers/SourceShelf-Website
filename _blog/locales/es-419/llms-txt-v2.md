...
La propuesta `llms.txt` ha alcanzado la versión 2. Esta actualización ayuda a que los asistentes y agentes de IA encuentren contenido web útil y legible por máquina. Si este formato es nuevo para ti, comienza con nuestra [introducción a llms.txt](what-is-llms-txt.md).
...
La versión 1.0.2 amplía el flujo de trabajo a Safari, permitiendo que la colección `llms.txt` de un sitio web compatible se convierta en el punto de partida para un Pack de investigación local. El [Guía de SourceShelf llms.txt](/docs/guides/llms-txt/) documenta el orden completo de descubrimiento, el flujo de trabajo de selección, las salvaguardias y la procedencia que la aplicación preserva.
...
Puede conservarlo como un archivo de investigación portátil, exportar Markdown para otra aplicación, crear un [Paquete de referencia orientado a la IA](/local-ai-reference-packs/) o exponer un paquete seleccionado a un cliente de IA compatible a través de la integración local de MCP de lectura única de SourceShelf.
